...
const bodySchema = z.object({
  instructions: z.string().trim().min(1, "Tell us what you want to say."),
})
...
export async function POST(
  ...
  try {
    const serverRes = await fetch(`${SERVER_URL}/prospects/reply/draft`, {
      method: "POST",
      headers: {
    ...
    const data = await serverRes.json().catch(() => ({ error: "Failed to generate draft." }))
