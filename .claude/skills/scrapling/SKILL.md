...
> This service is installed under `apps/scraper/` as a python library. It has their own hosted url for using it inside `apps/server` as `process.env.SCRAPER_URL` with `process.env.SCRAPER_API_KEY`
