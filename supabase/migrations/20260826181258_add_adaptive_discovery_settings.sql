-- Recovered from repository history after the migration directory was removed.
-- Production already has these objects, so this migration must be marked applied
-- there with `supabase migration repair` before the next database push. It remains
-- the canonical schema change for fresh/branch databases.
