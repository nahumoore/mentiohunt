...
export async function sendUnlinkedMentionAlertEmail({
  to,
  userId,
  userName,
  productName,
  ...
  await sendMentiohuntEmail({
    to,
    subject: `New unlinked mention opportunities for ${productName}`,
    unsubscribeUrl: generateUnsubscribeUrl(userId, "alerts"),
    previewText: `We found ${pluralize(prospectsCreated, "new unlinked mention opportunity", "new unlinked mention opportunities")} for ${productName}.`,
    footerReason:
      "You received this email because Mentiohunt found new unlinked mention opportunities for your product.",
    body: `
      ...
      <h1 style="font-family:-apple-system, BlinkMacSystemFont, 'Segoe UI', Helvetica, Arial, sans-serif; font-size:28px; color:#241611; margin:0 0 16px; line-height:1.2; letter-spacing:-0.5px;">New unlinked mention opportunities are ready</h1>
      <p style="font-family:-apple-system, BlinkMacSystemFont, 'Segoe UI', Helvetica, Arial, sans-serif; font-size:16px; color:#4F423A; margin:0 0 26px; line-height:1.7;">Today's scan for <strong style="color:#241611;">${escapedProductName}</strong> turned up pages that already mention your product but don't link to it yet.</p>
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="margin:0 -6px 10px;">
        <tr>${statCard("Unlinked mention opportunities", String(prospectsCreated), `pages mentioning ${productName} without a link`, "100%")}</tr>
      </table>
