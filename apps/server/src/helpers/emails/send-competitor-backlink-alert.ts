...
export async function sendCompetitorBacklinkAlertEmail({
  to,
  userId,
  userName,
  productName,
  ...
  await sendMentiohuntEmail({
    to,
    subject: `New competitor backlink opportunities for ${productName}`,
    unsubscribeUrl: generateUnsubscribeUrl(userId, "alerts"),
    previewText: `We found ${pluralize(prospectsCreated, "new competitor backlink opportunity", "new competitor backlink opportunities")} for ${productName}.`,
    footerReason:
      "You received this email because Mentiohunt found new competitor backlink opportunities for your product.",
    body: `
      ...
      <h1 style="font-family:-apple-system, BlinkMacSystemFont, 'Segoe UI', Helvetica, Arial, sans-serif; font-size:28px; color:#241611; margin:0 0 16px; line-height:1.2; letter-spacing:-0.5px;">New competitor backlink opportunities are ready</h1>
      <p style="font-family:-apple-system, BlinkMacSystemFont, 'Segoe UI', Helvetica, Arial, sans-serif; font-size:16px; color:#4F423A; margin:0 0 26px; line-height:1.7;">Today's scan for <strong style="color:#241611;">${escapedProductName}</strong> turned up sites that already link to your competitors and could link to you too.</p>
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="margin:0 -6px 10px;">
        <tr>${statCard("Competitor backlink opportunities", String(prospectsCreated), `sites linking to competitors where ${productName} could fit`, "100%")}</tr>
      </table>
