...
- **Severity: CRITICAL / MEDIUM / LOW, judged by business impact, not raw error count.** Mentiohunt's goal is finding backlink/mention opportunities and getting outreach sent — weigh each finding against that, not against how loud the log line looks:
  - **CRITICAL** — silently drops or corrupts prospects/opportunities/outreach sends, blocks a user from using the product (e.g. stuck logged out, dashboard errors), or risks shared sending-pool reputation (bounces, spam hits). Non-recovering + our-fault + user/opp-facing is almost always CRITICAL.
  - **MEDIUM** — degrades quality or volume of opportunities/outreach without fully blocking them (e.g. a scoring fallback chain exhausting on some batches, a scraper losing a few mentions to a fetch race, enrichment quietly skipping some contacts), or a self-healing issue that fires often enough to be a cost/reliability concern.
  - **LOW** — self-recovered, external-fault, already-ticketed-and-fixed, or infra/lint-level noise (advisor notices, slow-query logs) with no user or opportunity impact.
  Recovery + fault-attribution (from the prior bullets) feed this call, but the sort key is always "does this cost us opportunities/backlinks or hurt a user" — a frequent, self-healing, external-fault error can still rank below a rare CRITICAL one.
...
**6. Present the summary as a flat, severity-first listicle** — CRITICAL findings first, then MEDIUM, then LOW; skip empty tiers rather than writing "none". Each finding is one list item, bolded with a `[SEVERITY]` prefix and its service tag, e.g.:

- **[CRITICAL] Vercel/middleware** — what happened, self-recovered or not, our-fault vs external, ticket match status (existing ticket name, or "new").

Keep the per-service breakdown (Railway `server`, Railway `scraping`, Supabase `api`/`postgres`, advisories, Vercel `apps/web`) implicit via the service tag on each item rather than as separate section headers — severity order is the primary sort, not service. If the user's ask touched client-side behavior, note explicitly that Vercel/logcheck has no client-side error visibility rather than staying silent on it. Close by asking which of the new findings the user wants turned into tickets — do not create tickets unless asked.
