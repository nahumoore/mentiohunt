 ...
 *
 * Two steps: how the autopilot works, then what the prospect statuses mean.
 * `onDone` lets a caller run side effects (analytics, marking the walkthrough
 * seen) when the user finishes step 2 — omit it to just close the dialog.
 */
  ...
  return (
    <>
      {step === 1 ? (
        ...
        <>
          <DialogTitle>Prospect statuses</DialogTitle>
          <DialogDescription>
          ...
          <p className="mt-5 border-t border-border pt-4 text-xs leading-5 text-muted-foreground">
            Your only manual actions: dismiss a prospect you don&apos;t want,
            add a contact when one wasn&apos;t found, and reply once a
            conversation starts.
          </p>
        </>
      )}
      ...
      <div className="mt-6 flex items-center justify-between border-t border-border pt-4">
        ...
        {step === 1 ? (
          ...
          <DialogClose asChild>
            <Button className="rounded-full px-6 font-medium">Got it</Button>
          </DialogClose>
        )}
      </div>
    </>
