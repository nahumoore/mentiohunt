...
const VALID_SOURCES = ["email_inbox", "twitter", "bluesky"] as const
...
function validateMention(item: unknown): item is MediaMentionPayload {
  ...
  return (
    isValidSource(m.source) &&
    typeof m.source_ref === "string" &&
    m.source_ref.trim().length > 0
  )
}
