...
const loadingStages = [
  ...
  "Searching for sites that publish guest posts",
  "Scoring fit and building a rationale for each",
]

const proofPoints = [
  "Finds sites publicly inviting guest contributors — not a generic domain list.",
  "Each result shows the exact footprint that surfaced it.",
  "Built to turn a one-off search into a recurring outreach queue.",
]
...
export function GuestPostSitesFinder() {
  const [websiteUrl, setWebsiteUrl] = useState("")
  const [submittedUrl, setSubmittedUrl] = useState("")
  const [phase, setPhase] = useState<"idle" | "loading" | "results">("idle")
  const [stageIndex, setStageIndex] = useState(0)
  const [sites, setSites] = useState<GuestPostSite[]>([])
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    const [res] = await Promise.all([
      fetch("/api/free-tool/guest-post-sites-finder", {
        method: "POST",
          ...
          <div className="mt-8 rounded-2xl border border-border bg-card p-6 text-left">
            <form onSubmit={handleSubmit} className="space-y-3">
              ...
              <div className="flex flex-col gap-3 sm:flex-row">
                ...
                >
                  {phase === "loading" ? "Searching" : "Find guest post sites"}
                  {phase === "loading" ? (
            ...
            <div className="mt-5 rounded-xl border border-border bg-background p-4">
                ...
                <div className="flex gap-3">
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-sm leading-6 text-muted-foreground">
                      Enter your website URL to start searching for guest
              ...
              {hasResults ? (
                <div className="flex gap-3">
                  ...
                  <div>
                    <p className="font-heading text-base font-semibold tracking-[-0.03em]">
                      Search complete for {websiteDomain}.
                    </p>
                    <p className="mt-1 text-sm leading-6 text-muted-foreground">
          ...
          <div className="grid gap-8 lg:grid-cols-[0.72fr_1.28fr] lg:items-start">
            <div className="lg:sticky lg:top-28">
              ...
              <h2 className="mt-4 font-heading text-4xl font-semibold tracking-[-0.055em] text-balance sm:text-5xl">
                Sites worth pitching next.
              </h2>
              <p className="mt-5 max-w-md text-sm leading-7 text-muted-foreground">
                The search returns sites that publicly invite guest
                contributors in your niche — each paired with a fit rationale
                so you know why it surfaced.
              </p>
              ...
              {!hasResults ? (
                <div className="rounded-[2rem] border border-dashed border-[var(--color-blaze-orange)]/30 bg-card/70 p-8 text-center">
                  ...
                  <h3 className="mt-5 font-heading text-2xl font-semibold tracking-[-0.045em]">
                    Results appear after the search.
                  </h3>
                  <p className="mx-auto mt-3 max-w-md text-sm leading-7 text-muted-foreground">
                    Enter your website URL above to start the guest post
                    sites search.
                  </p>
                </div>
              ) : (
                  ...
                  {sites.length === 0 ? (
                    <div className="rounded-[2rem] border border-border bg-card/70 p-8 text-center">
                      <h3 className="font-heading text-2xl font-semibold tracking-[-0.045em]">
                        No guest post sites surfaced yet.
                      </h3>
                      <p className="mx-auto mt-3 max-w-md text-sm leading-7 text-muted-foreground">
                        We couldn&apos;t identify strong guest-posting targets
                        from this search. Sign up to unlock deeper discovery
                        with competitor analysis and keyword targeting.
                      </p>
                    </div>
                  ) : (
                    sites.map((site) => (
                        ...
                        <div className="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
                          <div className="flex min-w-0 gap-4">
                            <div className="flex size-12 shrink-0 items-center justify-center rounded-[1.15rem] border border-border bg-background shadow-sm">
                              <span
                                ...
                                style={{
                                  backgroundImage: `url(${getFaviconUrl(site.domain)})`,
                                }}
                              />
                            </div>
                          ...
                          <div className="flex flex-wrap gap-2 sm:justify-end">
                            <span className="inline-flex items-center gap-1.5 rounded-full border border-border bg-background/70 px-3 py-1 text-xs text-muted-foreground">
                              <IconArticle size={13} stroke={2.3} />
                              {site.matchedFootprint}
                            </span>
                          </div>
                        </div>
                  ...
                  <div className="relative overflow-hidden rounded-[2rem] border border-[var(--color-blaze-orange)]/25 bg-card p-6 shadow-[0_28px_90px_-54px_rgba(255,96,0,0.75)] sm:p-8">
                    ...
                    <div className="relative mx-auto max-w-xl text-center">
                      ...
                      <h3 className="mt-3 font-heading text-3xl font-semibold tracking-[-0.055em] text-balance">
                        Sign up to find more guest post targets and their
                        contact details.
                      </h3>
                      <p className="mx-auto mt-3 max-w-md text-sm leading-7 text-muted-foreground">
                        Get a recurring backlink opportunity queue built
                        around your product, keywords, and competitors — not
                        just a one-off search.
                      </p>
                      <Button
                        asChild
                        size="lg"
