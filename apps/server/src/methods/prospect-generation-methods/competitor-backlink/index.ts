...
export async function discoverCompetitorBacklinks(
  ...
  if (targetPagesError) {
    log.warn("failed to load customer target pages", { productId: product.id, error: targetPagesError.message })
  }
  const targetPages: CompetitorTargetPage[] = (rawTargetPages ?? []).map((page) => ({
    id: page.id,
    url: page.url,
    title: page.title,
  ...
  if (limits.refreshCompetitors === true) {
    ...
    const refreshDue = !lastRefresh
      || Date.now() - new Date(lastRefresh.refreshedAt).getTime() >= 7 * 24 * 60 * 60 * 1_000
    ...
    if (refreshDue && ownDomain) {
      try {
        ...
        log.warn("competitor refresh failed", { productId: product.id, error: String(error) })
      }
    }
  }

  const allDomains = [...new Set([...configuredDomains, ...inferredCompetitors])]

  if (allDomains.length === 0) {
    log.info("no valid competitors set, skipping", { productId: product.id })
    const runId = await createProspectRun(product.id, [])
    if (runId) {
      await completeProspectRun(runId, 0, competitorRefreshCostUsd, {}, {
        skip_reason: "no_valid_competitors",
        ...(competitorRefreshAt ? { competitor_refresh_at: competitorRefreshAt } : {}),
        ...(inferredCompetitors.length > 0 ? { inferred_competitors: inferredCompetitors } : {}),
  ...
  try {
    ...
    if (
      ...
      try {
        const ownDomain = extractDomainFromUrl(product.website_url)
        if (ownDomain) {
          const intersection = await getBacklinkPageIntersection(allDomains, ownDomain)
          ...
          for (const [competitorDomain, items] of byCompetitor) {
            if ((budget && budget.remaining <= 0) || limits.shouldStop?.()) break
            const uniqueItems = [...new Map(items.map((item) => [item.urlFrom, item])).values()]
        ...
        transportFailures += 1
        log.warn("backlink intersection failed", { productId: product.id, error: String(error) })
      }
    }

    if (runId) {
      await completeProspectRun(runId, totalProspectsCreated, totalCostUsd, mozCursorsByDomain, {
        ...funnel,
